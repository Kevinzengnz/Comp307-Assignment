...
def calculate_euclidean_distance(px, py, index1, index2):
    ...
    """
    return math.dist([px[index1], py[index1]], [px[index2], py[index2]])
...
def calculate_total_distance(routes, px, py, depot):
    ...
    for i, route in enumerate(routes):
        prev = depot
        for j, nodeIndex in enumerate(route):
            total += calculate_euclidean_distance(px, py, prev, nodeIndex)
            prev = nodeIndex
        total += calculate_euclidean_distance(px, py, route[-1], depot)
    return total
