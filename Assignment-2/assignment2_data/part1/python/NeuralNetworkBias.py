...
class Neural_Network_Bias:
    # Initialize the network
    def __init__(self, num_inputs, num_hidden, num_outputs, hidden_layer_weights, output_layer_weights, learning_rate, biases):
    ...
    def forward_pass(self, inputs):
        ...
        for i in range(self.num_hidden):
            ...
            for j in range(self.num_inputs):
                weighted_sum += self.hidden_layer_weights[j][i] * inputs[j] + self.biases[0][i]
        ...
        for i in range(self.num_outputs):
            ...
            for j in range(self.num_hidden):
                weighted_sum += self.output_layer_weights[j][i] * hidden_layer_outputs[j] + self.biases[1][i]
            output = self.sigmoid(weighted_sum)
    ...
    def backward_propagate_error(self, inputs, hidden_layer_outputs, output_layer_outputs, desired_outputs):
        ...
        delta_biases = np.copy(self.biases)
        for i in range(self.num_hidden):
            delta_biases[0][i] = self.learning_rate * hidden_layer_outputs[i] * (1 - hidden_layer_outputs[i]) * hidden_layer_betas[i]
        for i in range(self.num_outputs):
            delta_biases[1][i] = self.learning_rate * output_layer_outputs[i] * (1 - output_layer_outputs[i]) * output_layer_betas[i]
                       
        # Return the weights and biases we calculated, so they can be used to update all the weights and biases.
        return delta_output_layer_weights, delta_hidden_layer_weights, delta_biases
    ...
    def train(self, instances, desired_outputs, epochs):
        for epoch in range(epochs):
            ...
            for i, instance in enumerate(instances):
                ...
                delta_output_layer_weights, delta_hidden_layer_weights, delta_biases = self.backward_propagate_error(
